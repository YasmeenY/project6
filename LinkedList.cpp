...
{
  // if list size is less than 2 - nothing to do
  if( item_count_ < 2)
    return;
  // save head's next node
  Node<T> *next_ptr = head_ptr_->getNext();
  // set head's next node to null (it will be tail in inverted list)
  head_ptr_->setNext(nullptr);
  // call recursive method for rest of list
  invertRest(next_ptr);
} // end invert
...
void LinkedList<T>::rotate(int k)
{
  ...
  if(k < 0 )
  {
    std::string message = "rotate(int k) called with k < 0. Invalid parameter.";
    //throw(PrecondViolatedExcep(message));
	throw message;
  }
  // if list size is less than 2 or k = 0 - nothing to do
  if(item_count_ < 2 || k == 0 )
  {
    return;
  }

  Node<T> *current_ptr = head_ptr_;
  ...
  // move the last node to head
  current_ptr->setNext(head_ptr_);
  prev_ptr->setNext(nullptr);
  head_ptr_ = current_ptr;
  // call recursive method with k-1
  rotate(k - 1);
...
{
  // if reached the end of the list - return
  if(current_first_ptr == NULL)
    return;
  // save next node
  Node<T> *next_ptr = current_first_ptr->getNext();
  // insert current node to head
  current_first_ptr->setNext(head_ptr_);
  // current node becomes the head
  head_ptr_ = current_first_ptr;
  // call recursive method with next node (rest of list)
  invertRest(next_ptr);
